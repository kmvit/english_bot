...
class WhisperRecognizer:
    ...
    async def assess(self, wav_path: Path, duration_sec: float = 0.0) -> Assessment:
        ...
        transcript = await asyncio.to_thread(self._transcribe, model, wav_path)
        # Баллов произношения нет — блок в ответе учителя просто не появится.
        return Assessment(transcript=transcript, duration_sec=round(duration_sec, 2))

    def _transcribe(self, model: Any, wav_path: Path) -> str:
        try:
